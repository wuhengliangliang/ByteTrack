...
from yolox.data.transform import xyxy2xywh
...
class DetectionEngine:
    ...
    def __init__(self, config):
        ...
        # self.annFile = os.path.join(config.data_root, 'annotations/mini_mot.json') #val_half.json
        self.annFile = os.path.join(config.data_root, 'annotations/val_half.json')  # val_half.json
        self._coco = COCO(self.annFile)
        self.ids = self._coco.getImgIds()
        self.class_ids = sorted(self._coco.getCatIds())
    ...
    def _nms(self, xyxys, scores, threshold):
        ...
        ndets = xyxys.shape[0]
        supressed = np.zeros((ndets), np.int)
        keep = []
        for _i in range(ndets):
            ...
            for _j in range(_i + 1, ndets):
                ...
                h = max(0.0, yy2 - yy1)
                inter = w * h
                ovr = inter / (iareas + areas[j] - inter)
                if ovr >= threshold:
                    supressed[j] = 1
        return keep
    ...
    def get_eval_result(self):
        ...
        cocoEval.summarize()
        sys.stdout = stdout
        return rdct.content, cocoEval.stats[0], cocoEval.stats[1]

if __name__=='__main__':
    from model_utils.config import config
    from mindspore import context
    context.set_context(device_target="CPU", device_id=3)
    config.data_root = os.path.join('/disk0/zjc/dataset/mini_mot/')
    detect = DetectionEngine(config)
    outputs = np.load('/disk0/zjc/tools/output1.npy')
    out = detect.detection(outputs, [1080,1920], [1,1])
    print(out)
    print(out[0].shape)
    np.save("postprocess.npy", out[0])
